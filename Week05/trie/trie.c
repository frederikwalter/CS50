...
// check if name is in trie
bool check(char* word)
{
    // pointer for searching the trie
    node *ptr = root;

    // iterate through the word
    for (int i = 0, word_length = strlen(word); i < word_length; i++)
    {
        // return false if no further nodes along this path
        if (ptr -> children[tolower(word[i]) - 'a'] == NULL)
        {
            return false;
        }
        ptr = ptr -> children[tolower(word[i]) - 'a'];
    }
    // return true if final node is end of a valid name
    if (ptr -> is_word)
    {
        return true;
    }
